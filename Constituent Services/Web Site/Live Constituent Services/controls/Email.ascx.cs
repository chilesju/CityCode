namespace Constituent_Services.controls
{
	...
	public class Email : System.Web.UI.UserControl
	{
		...
		{
            string emailFrom = "me@topeka.org";// ConfigurationSettings.AppSettings[Global.CfgKeyEmailString];

            foreach (ListItem itm in emailList.Items)
            {
                if (itm.Selected == true)
                {
                    Utilites.User user = new Utilites.User("jchiles");
                    string emailTo = user.GetUsersEmail();
